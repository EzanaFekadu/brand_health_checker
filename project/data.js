...
// Two regional variants per product (US / EU), scores, reasons, nutrition,
// additives, provenance + a category of healthier alternatives.
window.BHC_DATA = (function () {

  const band = (s) => (s >= 80 ? 'GOOD' : s >= 70 ? 'OK' : s >= 45 ? 'FAIR' : 'POOR');
    ...
    '036632008015': {
      ...
      variants: {
        US: {
          ...
          additives: [
            { code: 'E1442', name: 'Modified starch', note: 'Thickener', flag: 'Both' },
          ],
  ...
  // Healthier alternatives (referenced by id)
  const alternatives = {
  ...
  // Recent scan history (pre-seeded)
  const history = ['009800895007', '049000028904', '036632008015'];

  // an unknown barcode used by the "not found" flow
  const unknownBarcode = '040000004271';

  return { products, alternatives, history, unknownBarcode, band };
})();
